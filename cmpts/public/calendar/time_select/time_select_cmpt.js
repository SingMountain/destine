...
Component({
	...
	methods: {
			...
			if (times == 0) {
				for (let k = this.data.startTimeStep; k <= this.data.endTimeStep; k++) {
					...
					let node = {
						...
						used: false,
						selected: false,
						expire: (day + ' ' + start < now)||(this.data.bookedStartTime<day + ' ' + start)&&(day + ' ' + start<this.data.bookedEndTime), //过期时间 
          }
      ...
      let day =this.data.day;
			if (!start || !end || !endPoint || !day) return;

			this.triggerEvent('select', { start, end, endPoint,day});
		}
	}
})
